...
class CocometroController {
  ...
  final ValueNotifier<bool> hasFailed = ValueNotifier(false); // Fim do jogo
  final ValueNotifier<bool> hasWon = ValueNotifier(false); // Vitória
  final ValueNotifier<bool> showNextLevelDialog = ValueNotifier(false);
  final ValueNotifier<bool> showGameOverDialog = ValueNotifier(false);
  final ValueNotifier<double> minRange =
      ValueNotifier(0.4); // Início do range ideal
  final ValueNotifier<double> maxRange =
      ValueNotifier(0.6); // Fim do range ideal
  final ValueNotifier<double> currentTimeInRange = ValueNotifier(0.0);
  final ValueNotifier<bool> isExcited =
      ValueNotifier(false); // Controle das imagens
  ...
  void _completeLevel() {
    coins.value += 5;
    level.value++;
    isExcited.value = true; // Mostra as imagens "excited_"
    Future.delayed(const Duration(seconds: 2), () {
      isExcited.value = false; // Retorna às imagens "sad_" após 2 segundos
    });
    _updateRange();
    resetProgress();
    showNextLevelDialog.value = true;
  }
  ...
  void resetProgress() {
    ...
    currentTimeInRange.value = 0.0;
    hasFailed.value = false;
    showNextLevelDialog.value = false;
    _startGameTimer();
  }
